    ...
    <section className="relative rounded-xl border border-gray-200 shadow-sm overflow-hidden w-full h-full">
      ...
      <div className="relative p-5 flex flex-col h-full">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/times" className="text-sm font-semibold text-yellow-600 hover:text-yellow-500">
            Ver todos →
          </Link>
        </div>
        ...
        <div className="flex-1 flex items-center">
          <div className="overflow-x-auto w-full snap-x snap-mandatory scroll-smooth">
            <div className="grid grid-rows-2 grid-flow-col auto-cols-[90px] gap-x-4 gap-y-8">

              {clubs.map((club, i) => (
                <Link key={i} href={`/clubs/${club.slug}`}>
                  <div className="flex flex-col items-center group snap-start">
                    ...
                    <div className="relative w-16 h-16">
                      <Image
                        ...
                        fill
                        sizes="64px"
                        className="object-contain rounded-full border border-gray-200 shadow-sm group-hover:shadow-md transition"
                      />
                    </div>

                    <span className="mt-2 text-[11px] text-gray-600 text-center">
                      {club.name}
                    </span>
